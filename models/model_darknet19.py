...
class model_darknet19:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope(self.name):
            ...
            hidden_layer1 = tf.nn.conv2d(X_input, W1, [1, 1, 1, 1], padding='SAME', name='hidden_layer1') + Bias1
            ...
            hidden_layer3 = tf.nn.conv2d(pool_layer2, W3, [1, 1, 1, 1], padding='SAME', name='hidden_layer3') + Bias3
            ...
            # Layer4
            W4 = tf.Variable(tf.random_normal([1, 1, 128, 64], stddev=0.01))
            Bias4 = tf.Variable(tf.constant(0.1, shape=[64]))
            hidden_layer4 = tf.nn.conv2d(hidden_layer3, W4, [1, 1, 1, 1], padding='SAME', name='hidden_layer4') + Bias4
            ...
            hidden_layer5 = tf.nn.conv2d(hidden_layer4, W5, [1, 1, 1, 1], padding='SAME', name='hidden_layer5') + Bias5
            hidden_layer5 = contrib.layers.batch_norm(hidden_layer5, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer5 = tf.nn.relu(hidden_layer5)
            ...
            hidden_layer6 = tf.nn.conv2d(pool_layer5, W6, [1, 1, 1, 1], padding='SAME', name='hidden_layer6') + Bias6
            hidden_layer6 = contrib.layers.batch_norm(hidden_layer6, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer6 = tf.nn.relu(hidden_layer6)
            print(hidden_layer6)

            # Layer7
            W7 = tf.Variable(tf.random_normal([1, 1, 256, 128], stddev=0.01))
            Bias7 = tf.Variable(tf.constant(0.1, shape=[128]))
            hidden_layer7 = tf.nn.conv2d(hidden_layer6, W7, [1, 1, 1, 1], padding='SAME', name='hidden_layer7') + Bias7
            hidden_layer7 = contrib.layers.batch_norm(hidden_layer7, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer7 = tf.nn.relu(hidden_layer7)
            print(hidden_layer7)

            # Layer8
            W8 = tf.Variable(tf.random_normal([3, 3, 128, 256], stddev=0.01))
            Bias8 = tf.Variable(tf.constant(0.1, shape=[256]))
            hidden_layer8 = tf.nn.conv2d(hidden_layer7, W8, [1, 1, 1, 1], padding='SAME', name='hidden_layer8') + Bias8
            hidden_layer8 = contrib.layers.batch_norm(hidden_layer8, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer8 = tf.nn.relu(hidden_layer8)
            pool_layer8 = tf.nn.max_pool(hidden_layer8, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            print(pool_layer8)

            # Layer9
            W9 = tf.Variable(tf.random_normal([3, 3, 256, 512], stddev=0.01))
            Bias9 = tf.Variable(tf.constant(0.1, shape=[512]))
            hidden_layer9 = tf.nn.conv2d(pool_layer8, W9, [1, 1, 1, 1], padding='SAME', name='hidden_layer9') + Bias9
            hidden_layer9 = contrib.layers.batch_norm(hidden_layer9, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer9 = tf.nn.relu(hidden_layer9)
            print(hidden_layer9)

            # Layer10
            W10 = tf.Variable(tf.random_normal([1, 1, 512, 256], stddev=0.01))
            Bias10 = tf.Variable(tf.constant(0.1, shape=[256]))
            hidden_layer10 = tf.nn.conv2d(hidden_layer9, W10, [1, 1, 1, 1], padding='SAME', name='hidden_layer10') + Bias10
            hidden_layer10 = contrib.layers.batch_norm(hidden_layer10, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer10 = tf.nn.relu(hidden_layer10)
            print(hidden_layer10)

            # Layer11
            W11 = tf.Variable(tf.random_normal([3, 3, 256, 512], stddev=0.01))
            Bias11 = tf.Variable(tf.constant(0.1, shape=[512]))
            hidden_layer11 = tf.nn.conv2d(hidden_layer10, W11, [1, 1, 1, 1], padding='SAME', name='hidden_layer11') + Bias11
            hidden_layer11 = contrib.layers.batch_norm(hidden_layer11, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer11 = tf.nn.relu(hidden_layer11)
            print(hidden_layer11)

            # Layer12
            W12 = tf.Variable(tf.random_normal([1, 1, 512, 256], stddev=0.01))
            Bias12 = tf.Variable(tf.constant(0.1, shape=[256]))
            hidden_layer12 = tf.nn.conv2d(hidden_layer11, W12, [1, 1, 1, 1], padding='SAME', name='hidden_layer12') + Bias12
            hidden_layer12 = contrib.layers.batch_norm(hidden_layer12, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer12 = tf.nn.relu(hidden_layer12)
            print(hidden_layer12)

            # Layer13
            W13 = tf.Variable(tf.random_normal([3, 3, 256, 512], stddev=0.01))
            Bias13 = tf.Variable(tf.constant(0.1, shape=[512]))
            hidden_layer13 = tf.nn.conv2d(hidden_layer12, W13, [1, 1, 1, 1], padding='SAME', name='hidden_layer13') + Bias13
            hidden_layer13 = contrib.layers.batch_norm(hidden_layer13, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer13 = tf.nn.relu(hidden_layer13)
            pool_layer13 = tf.nn.max_pool(hidden_layer13, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
            print(pool_layer13)

            # Layer14
            W14 = tf.Variable(tf.random_normal([3, 3, 512, 1024], stddev=0.01))
            Bias14 = tf.Variable(tf.constant(0.1, shape=[1024]))
            hidden_layer14 = tf.nn.conv2d(pool_layer13, W14, [1, 1, 1, 1], padding='SAME', name='hidden_layer14') + Bias14
            hidden_layer14 = contrib.layers.batch_norm(hidden_layer14, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer14 = tf.nn.relu(hidden_layer14)
            print(hidden_layer14)

            # Layer15
            W15 = tf.Variable(tf.random_normal([1, 1, 1024, 512], stddev=0.01))
            Bias15 = tf.Variable(tf.constant(0.1, shape=[512]))
            hidden_layer15 = tf.nn.conv2d(hidden_layer14, W15, [1, 1, 1, 1], padding='SAME', name='hidden_layer15') + Bias15
            hidden_layer15 = contrib.layers.batch_norm(hidden_layer15, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer15 = tf.nn.relu(hidden_layer15)
            print(hidden_layer15)

            # Layer16
            W16 = tf.Variable(tf.random_normal([3, 3, 512, 1024], stddev=0.01))
            Bias16 = tf.Variable(tf.constant(0.1, shape=[1024]))
            hidden_layer16 = tf.nn.conv2d(hidden_layer15, W16, [1, 1, 1, 1], padding='SAME', name='hidden_layer16') + Bias16
            hidden_layer16 = contrib.layers.batch_norm(hidden_layer16, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer16 = tf.nn.relu(hidden_layer16)
            print(hidden_layer16)

            # Layer17
            W17 = tf.Variable(tf.random_normal([1, 1, 1024, 512], stddev=0.01))
            Bias17 = tf.Variable(tf.constant(0.1, shape=[512]))
            hidden_layer17 = tf.nn.conv2d(hidden_layer16, W17, [1, 1, 1, 1], padding='SAME', name='hidden_layer17') + Bias17
            hidden_layer17 = contrib.layers.batch_norm(hidden_layer17, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer17 = tf.nn.relu(hidden_layer17)
            print(hidden_layer17)

            # Layer18
            W18 = tf.Variable(tf.random_normal([3, 3, 512, 1024], stddev=0.01))
            Bias18 = tf.Variable(tf.constant(0.1, shape=[1024]))
            hidden_layer18 = tf.nn.conv2d(hidden_layer17, W18, [1, 1, 1, 1], padding='SAME', name='hidden_layer18') + Bias18
            hidden_layer18 = contrib.layers.batch_norm(hidden_layer18, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer18 = tf.nn.relu(hidden_layer18)
            print(hidden_layer18)

            # Layer19
            W19 = tf.Variable(tf.random_normal([1, 1, 1024, self.class_count], stddev=0.01))
            Bias19 = tf.Variable(tf.constant(0.1, shape=[self.class_count]))
            hidden_layer19 = tf.nn.conv2d(hidden_layer18, W19, [1, 1, 1, 1], padding='SAME', name='hidden_layer19') + Bias19
            hidden_layer19 = contrib.layers.batch_norm(hidden_layer19, center=True, scale=True, is_training=self.keep_layer)
            hidden_layer19 = tf.nn.relu(hidden_layer19)
            print(hidden_layer19)


            # Global Average Pooling
            average_polling = tf.reduce_mean(hidden_layer19, axis=(1, 2), name="GAP")
            print(average_polling)

            self.output = tf.nn.softmax(average_polling, -1, 'output');
            print(self.output)
            correct_prediction = tf.equal(tf.argmax(self.output, 1), tf.argmax(self.Y, 1))

            # define cost/loss & optimizer
            self.cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=average_polling, labels=self.Y))
