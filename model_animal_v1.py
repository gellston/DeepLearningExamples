...
class model_animal_v1:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope(self.name):
            ...
            drop_layer3 = tf.nn.dropout(pool_layer3, keep_prob=self.keep_layer)
            ...
            drop_layer6 = tf.nn.dropout(hidden_layer6, keep_prob=self.keep_layer)
            ...
            full_connect_layer2 = tf.layers.dense(full_connect_dropout1, self.class_count)

            self.output = tf.nn.softmax(full_connect_layer2, -1, 'output');
            correct_prediction = tf.equal(tf.argmax(self.output, 1), tf.argmax(self.Y, 1))

            # define cost/loss & optimizer
            self.cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=full_connect_layer2, labels=self.Y))
